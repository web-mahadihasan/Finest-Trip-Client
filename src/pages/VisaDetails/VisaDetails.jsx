...
const VisaDetails = () => {
  ...
  const handleSubmitApplication = (applicationData) => {
    fetch("https://finest-trip-server.vercel.app/submit-application", {
      method: "POST",
      headers: {
  ...
  return (
    <div>
      <PageBanner
        bgImg="https://i.ibb.co.com/RPbNqx9/Section-4.png"
        title={`Details About ` + countryName + " " + visaType}
        path="visa-details"
      />
      <div className="container mx-auto px-4 md:px-0 mt-16 min-h-screen">
        <Link
          to={-1}
          className="flex items-center gap-2 py-2 px-4 border w-fit rounded-md border-primary-light font-jost font-medium my-2"
        >
          <span>
            <GoArrowLeft size={18} />
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <img
              src={countryImage}
              alt={countryName}
              className="w-full h-[300px] md:h-[320px] xl:h-[380px] rounded-md"
            />
            <div className="flex items-center gap-1 my-5">
              <img src={location} alt="" className="w-9" />
              <p className="font-rubik text-3xl font-bold text-titleBlack">
                {countryName}
              </p>
            </div>
            <div className="pt-1 font-jost flex flex-wrap items-center gap-6">
              <p className="text-lg">
                <span className="font-medium text-titleBlack">
                  Processing Time:{" "}
                </span>
            ...
            <div className="my-8 space-y-3">
              <h3 className="font-rubik text-2xl font-medium text-titleBlack">
                Description:
              </h3>
              <p className="font-jost text-lg text-black/65">
                <span className="block">{description}</span>
                Passport Scan Copy: Clearly scanned Passport copy required.
                Minimum of 6 months validity required from the arrival date.
                Photo Specification: Passport Size Photo with White Background
                (clear scanned Copy required) Previous Visa copy: If traveler
                has previous travel history on countries like Thailand,
                Malaysia, Singapore,
              </p>
            </div>
            {/* Requirements docs  */}
            <div className="border p-5">
              ...
              <p className="font-rubik text-lg my-2">
                <span className="text-red-500">*</span>Required Documents for
                Visa
              </p>
              <div className="ml-10 my-3">
                {requiredDocuments.map((req, idx) => (
                  <li
                    key={idx}
                    className="font-jost text-black/75 text-lg list-inside"
                  >
                    {req}
                  </li>
                ))}
              ...
              <div>
                ...
                <ul className="space-y-3 ml-6">
                  {extraReqDoc.map((extraDoc, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-2 font-jost text-black/65 text-lg"
                    >
                      <span>
                        <GiCheckMark size={16} className="text-primary mt-1" />
                      </span>
                      <span>{extraDoc}</span>
                    </li>
                  ))}
          ...
          <aside className="">
            <div className="md:grid grid-cols-2 lg:grid-cols-1 gap-3">
              {/* Visa fee  */}
              <div
                className="text-center p-6 rounded-md space-y-3"
                style={{
                  background:
                    "linear-gradient(125deg, rgba(99, 171, 69, 0.1) 0%, rgba(251, 176, 59, 0.1) 100%)",
                }}
              >
                <h3 className="text-titleBlack font-rubik text-2xl font-medium">
                  Cost Summary
                </h3>
                <h4 className="text-2xl font-rubik text-primary font-semibold">
                  $ <span>{visaFee} /</span>{" "}
                  <span className="text-lg text-titleBlack/60 font-normal">
                    per visa
                  </span>
                </h4>
                <p className="text-sm font-jost text-black/65">
                  Arrange your trip in advance - book this Visa now!
                </p>
                <Button
                  onClick={handleApplyNow}
                  variant="filled"
                  className="bg-primary text-white font-jost tracking-wide text-sm font-medium flex items-center gap-1 mx-auto hover:bg-primary-dark duration-300"
                >
                  Apply Now <RxExternalLink size={18} />
                </Button>
              </div>
              {/* plan your trip  */}
              <div>
