...
const MyVisaApplication = () => {
  const [visaApplicationData, setVisaApplicationData] = useState([]);
  const { user } = useAuth();

  useEffect(() => {
    fetch(
      `https://finest-trip-server.vercel.app/my-visa-application/${user.email}`
    )
      .then((res) => res.json())
      .then((data) => setVisaApplicationData(data));
  }, []);
  ...
  const handleCancelApplication = (appliedId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://finest-trip-server.vercel.app/cancel-application/${appliedId}`,
          {
            method: "DELETE",
          }
        )
          ...
          .then((data) => {
            if (data.deletedCount > 0) {
              const remainingApplication = [...visaApplicationData].filter(
                (prevData) => prevData._id !== appliedId
              );
  ...
  const handleSearch = (query) => {
    ...
    const searchResult = [...searchQuery].filter(
      (prev) => prev.countryName.toLowerCase() === query.toLowerCase()
    );
  ...
  const handleVisaTypeFilter = (query) => {
    ...
    const filterResult = [...filterQuery].filter(
      (prev) => prev.visaType.toLowerCase() === query.toLowerCase()
    );
    setVisaApplicationData(filterResult);
  };
  const handleSortByPrice = (query) => {
    if(query ===   "accending"){
        const accendingVisa = [...visaApplicationData].sort((a, b) =>  a.visaFee - b.visaFee)
        setVisaApplicationData(accendingVisa)
    }
    if(query ===   "descending"){
        const accendingVisa = [...visaApplicationData].sort((a, b) =>  b.visaFee - a.visaFee)
        setVisaApplicationData(accendingVisa)
    }
}
  return (
    <div>
      <PageBanner
        bgImg="https://i.ibb.co.com/2KHJjqV/Section-3.png"
      ...
      <div className="container mx-auto px-4 grid col-span-1 xl:grid-cols-3 gap-6 my-24">
        <div className="col-span-2 xl:col-span-1">
          <AsideToolBar
            ...
            handleVisaTypeFilter={handleVisaTypeFilter}
            handleSortByPrice={handleSortByPrice}
          />
        </div>
        <div className="col-span-2 space-y-6">
          {visaApplicationData?.length > 0 ? (
            visaApplicationData?.map((visaData) => (
              <VisaApplicationCard
                key={visaData._id}
            ...
            <h3 className="text-3xl font-rubik font-medium text-red-600">
              No Visa Application Found.{" "}
              <Link
                to={"/all-visa"}
                className="text-blue-900 underline text-2xl"
              >
                See our visa offer
              </Link>{" "}
            </h3>
